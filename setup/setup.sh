...
source tendermint/pieces/common.sh
source bcbchain/pieces/common.sh

echo 
echo SETUP bcchain...
if [[ "$(isCorruptedNode)" != "true" ]] && [[ "$(isCompletedNode)" != "true" ]] ; then
    (cd bcbchain/pieces; bash install.sh)
else
    (cd bcbchain/pieces; bash update.sh)
fi
...
echo SETUP tmcore...
if [[ "$(isCorruptedNode)" != "true" ]] && [[ "$(isCompletedNode)" != "true" ]] ; then
    (cd tendermint/pieces; bash install.sh)
    if [[ $? -eq 1 ]]; then
        exit
    fi
    sudo su - tmcore -s /bin/bash -c "/usr/local/tmcore/bin/run.sh init"
else
    (cd tendermint/pieces; bash update.sh)
fi
